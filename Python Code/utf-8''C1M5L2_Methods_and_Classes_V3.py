...
class Elevator:
    ...
    def go_to(self, floor):
        ...
        if self.current < floor and floor <= self.top:
            self.current = self.current + (floor - self.current)
        elif self.current > floor and floor >= self.bottom:
            self.current = self.current - (self.current - floor)
        pass
    def __str__(self):
        return f"Current floor: {str(self.current)}"
